...
OpenCode requires an exact catalogued `provider/model`, verifies the requested effort against that model's verbose variant table and a matching listed credential, then launches the long-lived TUI with a per-launch agent whose `model` and `variant` carry the sampled pair (`opencode run` is a one-shot batch command and is never used for a preset worker); ordinary OpenCode launches retain their prior command.
...
A relaunch that changes the harness away from the sampled candidate's own harness resets the sampled model and effort (explicit `--model`/`--effort` still override) and drops the Pi-only fast request, and `bin/fm-control.sh` validates that replacement profile through `fm-spawn.sh --preflight` before it stops the running agent.
