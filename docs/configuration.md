...
A harness switch also clears the replaced harness's recorded tool version and runtime session before the replacement records only what its own live checks establish.
...
Pi cleanup sums the task-attributable session JSONL files whose recorded working directory matches the task worktree and whose start falls inside the launch window, and Grok and OpenCode cleanup read the authoritative local usage records those tools keep (Grok `usage.json`, OpenCode's session/message store); each collector labels fields it could not bind as unknown or partial rather than as zero.
