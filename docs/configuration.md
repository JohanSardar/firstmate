...
`observe --defect-origin` attributes a recorded `bug-found` or `bug-escaped` observation to `original-implementation-worker`, `validation-correction`, `pre-existing-code`, or `unknown` together with its basis: a bare quality status never implies an origin, an explicitly unattributed defect blocks a task-level origin, and the finish event's `quality.defect_attribution` stays unknown unless exactly one explicit origin was recorded.
