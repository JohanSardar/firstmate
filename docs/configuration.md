...
Pi requires an exact catalogued `provider/model` with configured provider credentials and proves the requested thinking level against the installed package's own model catalog through `bin/fm-pi-reasoning-probe.mjs`; the probe forces that resolution offline (`PI_OFFLINE=1` plus `allowNetwork:false`), and a level the model does not support stops the launch rather than letting Pi silently clamp it to a lower level.
...
Fast is allowed only when the ordered launch plan this spawn composes and delivers proves `--no-extensions`, the per-worker task extension as the last extension in that plan, and no other extension but the verified registration-only backend integration before it (`bin/fm-pi-launch-plan-lib.sh`); a preset that carries a fixed fast value therefore passes `--no-extensions` in the composed argv and names the generated task extension explicitly as its final extension, while a preset without a fixed fast value, and every ordinary launch, keeps discovery and its single `-e` shape. The guarantee is stated for that composed argv: the resolved `pi` entry point can itself be a launcher wrapper that prepends its own extension arguments, which would load before the task extension, so extension discovery is disabled for the arguments this spawn composes rather than unconditionally.
On the herdr backend that plan also names Herdr's own Pi integration ahead of the task extension, because `--no-extensions` would otherwise leave the worker invisible to Herdr's agent registry; the integration is verified from the file the launch will actually load as the marked, registration-only `pane.report_agent` reporter with no provider-request rewriter (`bin/fm-herdr-pi-registration-lib.sh`), and a missing or unverifiable file refuses the launch instead of producing an unobservable worker.
...
Grok requires `grok models` to report both the exact model and a login, and proves the requested effort against the selected model's own advertised menu in the installed CLI's fetched catalog (`<GROK_HOME>/models_cache.json`, `bin/fm-grok-effort-lib.sh`) because support varies by model: `grok-4.6` advertises `xhigh` while `grok-4.5` does not. An unprovable pair, a catalog from a different grok version, and a missing catalog all stop the launch instead of falling back to a guessed global range; ordinary dispatch retains its older compatible range.
Claude Code accepts only a model token quoted by `claude --help` (a documented alias or full name), verifies `claude auth status`, and passes its effort flag; an exact id the installed help does not quote is refused because Claude Code ships no authoritative token-free catalog that enumerates it (`bin/fm-claude-model-lib.sh`).
OpenCode requires an exact catalogued `provider/model`, verifies a requested effort against that model's verbose variant table and a matching listed credential, then launches the long-lived TUI with a per-launch agent carrying only the explicitly requested `model` and `variant` (`opencode run` is a one-shot batch command and is never used for a preset worker); the agent's explicit `permission` block is the single permission mechanism, so `--auto` is not passed, and a launch with no requested axis delivers the ordinary permission-only configuration with no agent override.
A missing model, variant, credential, or verified fast route stops the selected launch without trying another candidate.
Each launch records the validation basis it actually proved in `effective.basis`: `validated-launch-control` only when its requested axes passed those adapter checks, `unvalidated-target-default` when no axis was requested and the target tool's own defaults apply, `unvalidated-foreign-harness` for a harness outside the preset vocabulary, and `unknown` when the record carries no basis.
...
The delivery outcome is classified from the same landed evidence for every ship cleanup, ordinary or forced, never from whether the cleanup needed `--force`: a cleanup whose merged-PR, content-in-default, or local-default-containment proof holds records `landed`, one whose landing proofs all conclusively answer "not landed" records `discarded`, and one whose evidence could not be gathered records `unknown`, while the finish event's separate `cleanup.discard_authorized` records that the local copy was discarded under explicit discard authorization.
...
A relaunch re-mints that session identity, so cleanup aggregates and deduplicates every incarnation the launch ledger recorded, including incarnations that recorded no runtime session at all, and reports the observation unknown without a partial sum when any incarnation's local record is unavailable.
Both that aggregation and the finish event's explicit `totals` (launches, relaunches, retries) are scoped to the task's own generation token (`state/<id>.meta` `dispatch_generation`), which every relaunch preserves and a fresh spawn mints, so reusing a task id after teardown cannot absorb a previous task's incarnations; a retry is a fresh spawn that reused an already-sampled durable choice.
The generation token and the launch's `launch_kind` and `selection_reused` fields are recorded explicitly, and a task or launch record missing one of them reports incomplete totals and incomplete scoped usage evidence rather than inferring a generation, launch kind, or reuse from another field.
When a live extension observation and a local collector describe the same incarnation, the collector's completeness status, partial flag, reason, and measured effort win, the live record keeps the fields only it can know, and a model or effort disagreement is recorded rather than hidden.
A relaunch that changes the harness away from the sampled candidate's own harness resets the sampled model and effort (explicit `--model`/`--effort` still override) and drops the Pi-only fast request; an unrequested axis is the replacement tool's own default rather than the literal string `default`, so it is never validated as a requested setting or written into a launch control.
`bin/fm-control.sh` validates that replacement profile through `fm-spawn.sh --preflight` before it stops the running agent, and a switch to a harness outside the preset candidate vocabulary runs on that tool's own defaults with no sampled profile to validate.
A harness switch also clears the replaced harness's recorded tool version and runtime session before the replacement records only what its own live checks establish; the same clearing applies when the task returns to the sampled harness, whose sampled fast axis is then reconstructed from the durable choice rather than from the foreign incarnation's cleared record.
A relaunch that changes harness retires the replaced harness's runtime observation before delivery and reports cross-harness usage as unknown with an explicit reason, because a local session store from one harness is not a compatible unit with another's.
Claude transcript usage is counted once per assistant message id across the main transcript and every Agent-tool subagent transcript, because Claude Code repeats one response's usage on every content-block line and mirrors the spawning message into each subagent transcript; a usage line without that id, or an unreadable subagent transcript, keeps the usage observation unknown.
Pi cleanup sums the task-attributable session JSONL files whose recorded working directory matches the task worktree and whose start is at or after this generation's recorded dispatch start; there is no pre-launch allowance, so a reused slot's previous occupant stays out.
Grok and OpenCode cleanup read the authoritative local usage records those tools keep (Grok `usage.json`, OpenCode's session/message store); each collector labels fields it could not bind as unknown or partial rather than as zero, and every collector, including Grok, carries an explicit unknown usage block with its own precise per-incarnation reason when its local record is unavailable or partial.
Later model, fast-verification, token usage, quality, and defect-origin observations can be appended with `fm-dispatch-metrics.sh observe`; its help owns the argument shape.
Every observation is stamped with the task's generation token, either explicitly through `--generation` or otherwise from the newest launch-prepared record for that task, and a finish event aggregates only defect evidence carrying its own generation: observations from another generation or with no token at all are preserved as `unscoped_evidence` and never inherited by a reused task id.
`observe --defect-origin` attributes a recorded `bug-found` or `bug-escaped` observation to `original-implementation-worker`, `validation-correction`, `pre-existing-code`, or `unknown` together with its basis: a bare quality status never implies an origin, any recorded defect whose origin is absent or unknown blocks a task-level attribution, and the finish event's `quality.defect_attribution` stays unknown unless exactly one known origin was recorded and no recorded defect lacks a proven origin.
