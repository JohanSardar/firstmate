...
Fast is allowed only when the delivered ordered launch plan proves `--no-extensions`, the per-worker task extension as the last extension, and no other extension but the verified registration-only backend integration before it (`bin/fm-pi-launch-plan-lib.sh`); a preset that carries a fixed fast value therefore disables Pi's extension discovery and names the generated task extension explicitly, while a preset without a fixed fast value, and every ordinary launch, keeps discovery and its single `-e` shape.
...
The delivery outcome is classified from landed evidence, never from whether the cleanup needed `--force`: a forced cleanup whose merged-PR or content-in-default proof holds records `landed`, one whose landing proofs both conclusively answer "not landed" records `discarded`, and one whose evidence could not be gathered records `unknown`, while the finish event's separate `cleanup.discard_authorized` records that the local copy was discarded under explicit discard authorization.
