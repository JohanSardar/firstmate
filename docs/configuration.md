...
Fast is allowed only when the delivered ordered launch plan proves `--no-extensions` with the per-worker task extension as the last extension (`bin/fm-pi-launch-plan-lib.sh`); every preset Pi launch therefore disables Pi's extension discovery and names the generated task extension explicitly, while an ordinary launch keeps discovery and its single `-e` shape.
