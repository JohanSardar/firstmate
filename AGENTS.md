...
config/task-model-presets.json  optional explicit fixed/weighted task-model experiments; LOCAL, gitignored, inert unless a fresh task names one with fm-spawn --preset, and NOT inherited; docs/configuration.md "Task/model presets" owns schema, selection, and metrics
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  dispatch-metrics.jsonl private append-only requested/effective settings, selection provenance, duration, observations, and estimate-labeled comparison events for opt-in task/model presets
...
state/               runtime records and signals; gitignored
  ...
  <id>.dispatch-choice.json <id>.dispatch-runtime.json  opt-in task/model preset sample and observed runtime settings; removed after the durable comparison event is finalized, and a replayed pending close retires the pair a refused close left without a task record
...
A current explicit experiment may instead name one opt-in `config/task-model-presets.json` entry through `fm-spawn --preset`; do not activate, infer, or auto-match these presets, and never substitute them for ordinary dispatch or quota-ranked arrays.
The preset owner records deterministic fixed/weighted provenance before launch; a sampled unavailable candidate or unverified live setting stops without fallback or weight renormalization.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `PRESENTATION_UNAVAILABLE:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `STARTUP_MEMORY_BUDGET:`, `CREW_DISPATCH: invalid`, `TASK_MODEL_PRESETS: invalid`, `FLEET_SYNC:`, `NETWORK_CHECKS:`, `HOME_SUMMARY:`, `BACKLOG_RECONCILE:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `SECONDMATE_HANDOFF:`, `NUDGE_SECONDMATES:`, or `FMX:`), or when `BOOTSTRAP_INFO:` says an interrupted backlog cleanup may have left an endpoint or local copy; silence and other `BOOTSTRAP_INFO:` facts need no load.
