...
config/task-model-presets.json  optional explicit fixed/weighted task-model experiments; LOCAL, gitignored, and NOT inherited; this delivery provides schema and deterministic selection only, while fm-spawn --preset activation and the metrics ledger are forward-looking and owned by the separate executable-adapter and metrics delivery; docs/configuration.md "Task/model presets" owns the contract
...
A separate executable-adapter and metrics delivery will let a current explicit experiment name one opt-in `config/task-model-presets.json` entry through `fm-spawn --preset`; until it lands no spawn reads this file, and these presets must never be activated, inferred, or auto-matched in place of ordinary dispatch or quota-ranked arrays.
The selector records deterministic fixed/weighted provenance for every sample; a sampled unavailable candidate stops selection without fallback or weight renormalization.
