...
config/task-model-presets.json  optional opt-in fixed/weighted task-model experiment presets; LOCAL, gitignored, and NOT inherited; no spawn reads it - bootstrap only validates it and bin/fm-task-model-preset.sh selects from it only when run explicitly; docs/configuration.md "Task/model presets" owns the contract
...
`config/task-model-presets.json` presets are an opt-in experiment that no spawn reads: never activate, infer, or auto-match one in place of ordinary dispatch or a quota-ranked array, and never select around a sampled unavailable candidate; `docs/configuration.md` "Task/model presets" owns the schema and selection contract.
