...
write_task_model_presets() {
  local home=$1
  cat > "$home/config/task-model-presets.json" <<'JSON'
{"schema_version":1,"seed":"spawn-test-seed","presets":{"review-fixed":{"mode":"fixed","candidate":{"id":"fixed","harness":"pi","model":"vendor/model-fixed","effort":"high","fast":false}},"impl-weighted":{"mode":"weighted","candidates":[{"id":"grok-choice","weight":0.5,"harness":"grok","model":"grok-4","effort":"high"},{"id":"claude-choice","weight":0.5,"harness":"claude","model":"opus","effort":"medium"}]},"blocked":{"mode":"fixed","candidate":{"id":"pending-product","harness":"opencode","model":"vendor/model-pending","effort":"xhigh","available":false,"unavailable_reason":"product approval is pending"}},"fast-pi":{"mode":"fixed","candidate":{"id":"fast","harness":"pi","model":"vendor/model-fast","effort":"high","fast":true}}}}
JSON
}

test_preset_drives_scout_launch_and_records_the_choice() {
  local rec id out status launch choice
  id=preset-scout-z30
  rec=$(make_spawn_case preset-scout claude "$id")
  read_case_record "$rec"
  write_task_model_presets "$HOME_DIR"

  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --scout --preset review-fixed)
  status=$?
  expect_code 0 "$status" "scout spawn naming a fixed preset should launch"
  assert_contains "$out" "spawned $id harness=pi kind=scout" "the preset's harness did not drive the scout launch"
  assert_meta_profile "$HOME_DIR/state/$id.meta" pi vendor/model-fixed high
  assert_grep "preset=review-fixed" "$HOME_DIR/state/$id.meta" "meta did not record the preset that chose the profile"
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "FM_PI_HARNESS=pi '$FAKEBIN_DIR/pi' --tui-mode regular --model 'vendor/model-fixed' --thinking 'high' -e" \
    "the preset's model and effort did not reach the Pi launch"
  choice="$HOME_DIR/state/$id.dispatch-choice.json"
  assert_present "$choice" "the durable preset choice was not published"
  assert_equals "review-fixed:fixed:fixed" "$(jq -r '.preset + ":" + .mode + ":" + .selected.id' "$choice")" \
    "durable choice does not carry the sampled preset and candidate"
  pass "--preset drives a scout launch from a fixed preset and records the choice"
}

test_preset_drives_ship_launch_from_the_reproducible_weighted_sample() {
  local rec id out status launch harness model effort
  id=preset-ship-z31
  rec=$(make_spawn_case preset-ship claude "$id")
  read_case_record "$rec"
  write_task_model_presets "$HOME_DIR"
  # The same seed, preset, and task id must sample the same candidate anywhere,
  # so an independent selector run predicts exactly which worker the spawn
  # launches; the spawn's own record must match it byte for byte.
  mkdir -p "$CASE_DIR/independent-state"
  FM_STATE_OVERRIDE="$CASE_DIR/independent-state" "$ROOT/bin/fm-task-model-preset.sh" select "$id" impl-weighted \
    "$HOME_DIR/config/task-model-presets.json" > "$CASE_DIR/expected-choice.json" \
    || fail "independent selector run failed"
  harness=$(jq -r '.selected.harness' "$CASE_DIR/expected-choice.json")
  model=$(jq -r '.selected.model' "$CASE_DIR/expected-choice.json")
  effort=$(jq -r '.selected.effort' "$CASE_DIR/expected-choice.json")

  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --preset impl-weighted)
  status=$?
  expect_code 0 "$status" "ship spawn naming a weighted preset should launch"
  assert_contains "$out" "spawned $id harness=$harness kind=ship" "the sampled harness did not drive the ship launch"
  assert_meta_profile "$HOME_DIR/state/$id.meta" "$harness" "$model" "$effort"
  assert_grep "preset=impl-weighted" "$HOME_DIR/state/$id.meta" "meta did not record the weighted preset"
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "--model '$model'" "the sampled model did not reach the launch"
  cmp -s "$CASE_DIR/expected-choice.json" "$HOME_DIR/state/$id.dispatch-choice.json" \
    || fail "the spawn's durable choice differs from the independent selector's record"
  pass "--preset launches the ship worker the reproducible weighted sample names"
}

test_preset_refusals_launch_nothing_and_never_fall_through() {
  local rec id out status
  rec=$(make_spawn_case preset-refuse claude preset-unknown-z32 preset-blocked-z33 preset-fast-z34 preset-axes-z35 preset-positional-z36)
  read_case_record "$rec"
  write_task_model_presets "$HOME_DIR"

  id=preset-unknown-z32
  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --scout --preset nope)
  status=$?
  expect_code 1 "$status" "an unknown preset must refuse the spawn"
  assert_contains "$out" "task/model preset 'nope' is not configured" "unknown preset refusal was not actionable"
  [ ! -s "$LAUNCH_LOG" ] || fail "an unknown preset must launch nothing (got: $(cat "$LAUNCH_LOG"))"
  assert_absent "$HOME_DIR/state/$id.meta" "an unknown preset must not fall through to crew-harness"
  assert_absent "$HOME_DIR/state/$id.dispatch-choice.json" "an unknown preset must not publish a choice"

  id=preset-blocked-z33
  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --scout --preset blocked)
  status=$?
  expect_code 1 "$status" "a sampled unavailable candidate must refuse the spawn"
  assert_contains "$out" "sampled unavailable candidate 'pending-product': product approval is pending" \
    "unavailable refusal did not carry the candidate and its reason"
  [ ! -s "$LAUNCH_LOG" ] || fail "an unavailable candidate must launch nothing (got: $(cat "$LAUNCH_LOG"))"
  assert_absent "$HOME_DIR/state/$id.meta" "an unavailable candidate must not be substituted"
  assert_equals pending-product "$(jq -r '.selected.id' "$HOME_DIR/state/$id.dispatch-choice.json")" \
    "the unavailable sample must be retained for a later approval"

  id=preset-fast-z34
  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --scout --preset fast-pi)
  status=$?
  expect_code 1 "$status" "a sampled Pi fast setting with no verified launch flag must refuse the spawn"
  assert_contains "$out" "candidate 'fast' with fast=true" "fast refusal did not name the sampled candidate"
  [ ! -s "$LAUNCH_LOG" ] || fail "a fast candidate must not launch at the default speed (got: $(cat "$LAUNCH_LOG"))"
  assert_absent "$HOME_DIR/state/$id.meta" "a fast candidate must not launch without its speed setting"

  id=preset-axes-z35
  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --scout --preset review-fixed --model other)
  status=$?
  expect_code 1 "$status" "--preset with an explicit profile axis must refuse"
  assert_contains "$out" "--preset owns the harness, model, and effort axes" "axis contradiction refusal missing"
  assert_absent "$HOME_DIR/state/$id.dispatch-choice.json" "a contradicted preset must not sample"

  id=preset-positional-z36
  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" codex --scout --preset review-fixed)
  status=$?
  expect_code 1 "$status" "--preset with a positional harness must refuse"
  assert_contains "$out" "drop the positional harness or launch command" "positional harness refusal missing"
  assert_absent "$HOME_DIR/state/$id.dispatch-choice.json" "a contradicted preset must not sample"

  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" preset-secondmate-z37 --secondmate --preset review-fixed)
  status=$?
  expect_code 1 "$status" "--preset on a secondmate spawn must refuse"
  assert_contains "$out" "--preset applies only to ship and scout spawns" "secondmate refusal missing"

  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" preset-relaunch-z38 --relaunch --preset review-fixed)
  status=$?
  expect_code 1 "$status" "--preset on a relaunch must refuse"
  assert_contains "$out" "--relaunch reuses the task's recorded profile" "relaunch refusal missing"
  [ ! -s "$LAUNCH_LOG" ] || fail "contradicted presets must launch nothing (got: $(cat "$LAUNCH_LOG"))"
  pass "preset refusals stop before any launch, metadata, or fallback dispatch"
}

test_batch_forwards_preset_and_satisfies_the_dispatch_profile_backstop() {
  local rec id1 id2 out status
  id1=preset-batch-a-z39
  id2=preset-batch-b-z40
  rec=$(make_spawn_case preset-batch claude "$id1" "$id2")
  read_case_record "$rec"
  write_task_model_presets "$HOME_DIR"
  enable_dispatch_profile "$HOME_DIR"

  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" \
    "$id1=$PROJ_DIR" "$id2=$PROJ_DIR" --preset review-fixed)
  status=$?
  expect_code 0 "$status" "batch spawn with a shared preset should succeed even under an active dispatch profile"
  assert_contains "$out" "spawned $id1 harness=pi" "first batch task did not launch on the preset"
  assert_contains "$out" "spawned $id2 harness=pi" "second batch task did not launch on the preset"
  assert_meta_profile "$HOME_DIR/state/$id1.meta" pi vendor/model-fixed high
  assert_meta_profile "$HOME_DIR/state/$id2.meta" pi vendor/model-fixed high
  assert_grep "preset=review-fixed" "$HOME_DIR/state/$id1.meta" "first batch meta lost the preset"
  assert_grep "preset=review-fixed" "$HOME_DIR/state/$id2.meta" "second batch meta lost the preset"
  assert_present "$HOME_DIR/state/$id1.dispatch-choice.json" "first batch task has no durable choice"
  assert_present "$HOME_DIR/state/$id2.dispatch-choice.json" "second batch task has no durable choice"
  pass "batch dispatch forwards --preset to every pair as the explicit harness source"
}
