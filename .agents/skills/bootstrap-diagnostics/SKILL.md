...
- `TASK_MODEL_PRESETS: invalid config/task-model-presets.json - <reason>` - the opt-in experiment file is malformed; do not activate a preset, resample, or select around the error, while ordinary dispatch remains available because this file is never implicit.
