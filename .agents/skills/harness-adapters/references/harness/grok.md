...
| Effort | Ordinary dispatch keeps the compatible `--reasoning-effort <low\|medium\|high>` range because version 0.2.99 rejects xhigh and max. An explicit task/model preset proves the requested level against the selected model's own advertised menu in the CLI's fetched catalog (`<GROK_HOME>/models_cache.json`, `bin/fm-grok-effort-lib.sh`) because support varies by model: on 1.0.30 `grok-4.6` advertises xhigh while `grok-4.5` does not. An unprovable pair, a catalog from a different grok version, and a missing catalog refuse before launch; max remains unsupported, and `references/common/model-and-effort.md` owns fallback and unsupported-value handling. |
