...
Pi invokes `before_provider_request` handlers in extension load order and only the last registered handler owns the payload, so this extension registers its handler once when it loads and `bin/fm-spawn.sh` allows a fixed `fast` value only when the delivered ordered launch plan proves `--no-extensions` with this task extension last (`bin/fm-pi-launch-plan-lib.sh`).
A preset launch that carries a fixed fast value delivers exactly that plan - discovery disabled, the generated task extension named last - so the request is launchable and the ledger still records it as requested-only; a plan that cannot prove the ordering is refused before launch rather than claimed.
