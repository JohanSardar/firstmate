...
Every preset Pi launch delivers exactly that plan - discovery disabled, the generated task extension named last - so a fixed fast request is launchable and the ledger still records it as requested-only; a plan that cannot prove the ordering is refused before launch rather than claimed.
