...
| Effort flag | `--thinking <low\|medium\|high\|xhigh\|max>`; both identities expose the same levels and completed the same model-qualified max-thinking smoke. An opt-in preset proves the exact requested level against the installed package's own local model catalog (`bin/fm-pi-reasoning-probe.mjs`, which disables catalog network refresh because Pi clamps an unsupported level silently). |
...
Pi invokes `before_provider_request` handlers in extension load order and only the last registered handler owns the payload, so this extension registers its handler once when it loads and `bin/fm-spawn.sh` allows a fixed `fast` value only when the ordered launch plan it composes proves `--no-extensions` with this task extension last and no other extension but the verified registration-only backend integration before it (`bin/fm-pi-launch-plan-lib.sh`).
A preset launch that carries a fixed fast value composes exactly that plan - `--no-extensions`, the generated task extension named last, and on the herdr backend Herdr's marked, registration-only Pi integration ahead of it so the worker stays observable (`bin/fm-herdr-pi-registration-lib.sh`) - and launches that argv, so the request is launchable and the ledger still records it as requested-only; a plan that cannot prove the ordering or the backend integration is refused before launch rather than claimed.
The guarantee covers the argv `bin/fm-spawn.sh` composes: the resolved `pi` entry point can itself be a launcher wrapper that prepends its own extension arguments, which would load before the task extension, so extension discovery is disabled for the composed arguments plus the task extension's final position in them, not extensions a wrapper injects outside that argv.
