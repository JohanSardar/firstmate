...
Pi invokes `before_provider_request` handlers in extension load order and only the last registered handler owns the payload, so this extension registers its handler once when it loads and `bin/fm-spawn.sh` allows a fixed `fast` value only when the delivered ordered launch plan proves `--no-extensions` with this task extension last and no other extension but the verified registration-only backend integration before it (`bin/fm-pi-launch-plan-lib.sh`).
A preset launch that carries a fixed fast value delivers exactly that plan - discovery disabled, the generated task extension named last, and on the herdr backend Herdr's marked, registration-only Pi integration ahead of it so the worker stays observable (`bin/fm-herdr-pi-registration-lib.sh`) - so the request is launchable and the ledger still records it as requested-only; a plan that cannot prove the ordering or the backend integration is refused before launch rather than claimed.
